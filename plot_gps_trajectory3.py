...
実行例:
    python plot_gps_trajectory3.py logs/mission_20260804_181539_history.csv

矢印なし:
    python plot_gps_trajectory3.py logs/mission_20260805_082813_history.csv --no-heading
...
def common_coordinate_offset(
    minimum: float,
    maximum: float,
    max_decimal_places: int = 10,
) -> tuple[float, int]:
    """
    座標範囲に共通する先頭部分をオフセットとして求める。
    ...
    if math.isclose(lower, upper, rel_tol=0.0, abs_tol=1.0e-15):
        return 0.0, 0

    common_decimal_places = 0

    for decimal_places in range(max_decimal_places + 1):
        scale = 10**decimal_places

        lower_prefix = math.floor(lower * scale)
        upper_prefix = math.floor(upper * scale)
    ...
    scale = 10**common_decimal_places
    offset = math.floor(lower * scale) / scale

    return offset, common_decimal_places


def tick_decimal_places(ticks: list[float], maximum: int = 10) -> int:
    """
    隣接する目盛間隔から、必要な小数点以下の桁数を決定する。
    """
    differences = [
        abs(second - first)
        for first, second in zip(ticks, ticks[1:])
        if not math.isclose(first, second, rel_tol=0.0, abs_tol=1.0e-15)
    ]

    if not differences:
        return 6

    minimum_step = min(differences)

    if minimum_step >= 1.0:
        return 0

    decimal_places = math.ceil(-math.log10(minimum_step))

    # 0.00005のような間隔を正しく区別できるように余裕を持たせる。
    scaled_step = minimum_step * (10**decimal_places)
    if not math.isclose(
        scaled_step,
        round(scaled_step),
        rel_tol=0.0,
    ...
    return min(maximum, max(0, decimal_places))


def format_coordinate_axis(
    axis: Axis,
    axis_name: str,
    minimum: float,
    maximum: float,
    ...
    """
    緯度・経度軸の共通部分を軸ラベルへ移し、
    目盛にはオフセットからの差分だけを表示する。

    表示例:
        軸ラベル:
            Longitude [deg] + 139.909
    ...
    ticks = list(locator.tick_values(minimum, maximum))
    visible_ticks = [
        tick
        for tick in ticks
        if minimum - 1.0e-12 <= tick <= maximum + 1.0e-12
    ]

    if len(visible_ticks) >= 2:
        ticks_for_precision = visible_ticks
    else:
        ticks_for_precision = ticks

    decimal_places = tick_decimal_places(ticks_for_precision)

    offset, offset_decimal_places = common_coordinate_offset(
        minimum,
        maximum,
    )

    # 共通部分が整数部だけの場合も含めて、十分な効果がある場合に
    # オフセット表示を使用する。
    use_offset = not math.isclose(
        offset,
        0.0,
        rel_tol=0.0,
        abs_tol=1.0e-15,
    ...
    if use_offset:
        offset_text = f"{offset:.{offset_decimal_places}f}"

        if offset >= 0.0:
            label = f"{axis_name} [deg] + {offset_text}"
        else:
            label = f"{axis_name} [deg] - {abs(offset):.{offset_decimal_places}f}"
        ...
        axis.set_major_formatter(
            FuncFormatter(
                lambda value, position: (
                    f"{value - offset:.{decimal_places}f}"
                )
            )
        )
    else:
        axis.set_label_text(f"{axis_name} [deg]")
        axis.set_major_formatter(
            FuncFormatter(
                lambda value, position: f"{value:.{decimal_places}f}"
            )
        )
