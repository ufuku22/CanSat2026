...
from config import ArlissMissionConfig  # noqa: E402
from drive_controller import DriveController  # noqa: E402
from logger import Logger  # noqa: E402
from mission_arliss import (  # noqa: E402
    TARGET_LATITUDE_DEG,
...
LOG_FILE = "arliss_selfie_mission_test.log"


def main() -> int:
    """IMU・モーター・自撮り機能だけをARLISS設定で1回実行する。"""
    ...
    logger.event(
        "ARLISS自撮りミッションテスト開始 "
        f"(接続待ちタイムアウト="
        f"{ArlissMissionConfig.SELFIE_CONNECTION_TIMEOUT_S:.1f}秒)"
    )
    try:
        ...
        telemetry = TelemetryService(
            sensors,
            logger,
            interval_s=ArlissMissionConfig.TELEMETRY_INTERVAL_S,
        )
        ...
        logger.event("ARLISS自撮りミッションテスト中断")
        exit_code = 130
    except (Exception, SystemExit) as exc:
        logger.event(
            f"ARLISS自撮りミッションテスト失敗 "
