...
def log_sensors(
    ...
    """停止指示を受けるまで、環境センサと9軸センサの値をCSVへ記録する。"""
...
def check_modules(
    ...
    try:
        ...
        logger.event(f"Radio transmission check: {'OK' if radio_ok else 'NG'}")
    ...
    try:
        ...
        logger.event(
            ...
            f"(has_fix={gnss.get('has_fix')}, satellites={gnss.get('satellites')})"
        )
    except Exception as exc:
...
def main() -> None:
    ...
    try:
        ...
        # 2. CSV記録を別スレッドで開始し、振動試験時間の3分間待機する。
        logger.event("Sensor measurement started")
        sensor_thread = threading.Thread(
            target=log_sensors,
            args=(sensors, sensor_log_path, stop_event),
            daemon=True,
        )
        sensor_thread.start()
