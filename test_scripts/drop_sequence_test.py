...
def log_sensors(
    ...
    with CsvLogger(
        ...
        while not stop_event.is_set():
            now = time.monotonic()
            if (
                now < next_sample_time
                and stop_event.wait(next_sample_time - now)
            ):
                break
            ...
            row = csv_logger.write_row({"altitude_m": f"{altitude_m:.4f}"})
            if display_event.is_set():
                print(
                    "Sensor: "
                    f"temp={row['temperature_c']}C, "
                    f"pressure={row['pressure_hpa']}hPa, "
                    f"altitude_m={row['altitude_m']}m, "
...
def main() -> None:
    ...
    try:
        ...
        try:
            ...
            ground_pressure_hpa = read_median_pressure_hpa(
                sensors,
        ...
        released = judge_release(
            ...
            timeout_s=None,
            measurement_interval_s=SENSOR_INTERVAL_SECONDS,
            on_third_threshold=lambda pressure_hpa: send_release_pressure(
                pressure_hpa,
                logger,
            ),
        )
